...
class ExpandingSectionsVC: UIViewController {
...
extension ExpandingSectionsVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if isSectionOpen.count >= section {
            if(isSectionOpen[section]) {
                return self.dataSections[section].data.count
            }
        }
        return 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let brand = dataSections[indexPath.section].data[indexPath.row]
    ...
    @objc func headerTapped(sender: UITapGestureRecognizer) {
        if let linkSection = sender.view {
            ...
            if(isSectionOpen[section]) {
                isSectionOpen[section] = false
                self.dataTableView.reloadData()
                
            } else {
                self.isSectionOpen[section] = true
                self.dataTableView.reloadData()
            }
        }
    }
